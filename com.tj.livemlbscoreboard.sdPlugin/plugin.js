...
function renderButton(context, game) {
    ...
    if (game && game.state === 'live' && game.gamePk) {
...
function buildLines(game) {
    ...
    switch (game.state) {
        ...
        case 'ppd':     return [game.matchup, 'PPD'];
        case 'susp':    return [game.matchup, 'SUSP'];
        case 'delay':   return [game.matchup, 'DELAY'];
        case 'live':    return [
...
function parseAllGames(data) {
    try {
        ...
        return games.map(g => {
            ...
            if (detailed.includes('Delayed'))   return { state: 'delay', matchup, gamePk, gameDate, startISO, homeId, awayId };
